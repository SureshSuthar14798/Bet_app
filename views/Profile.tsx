...
const ProfilePage: React.FC<ProfilePageProps> = ({ onNavigate }) => {
  ...
  return (
    <MDiv 
      className="max-w-6xl mx-auto py-6 space-y-6 w-full"
    ...
    >
      ...
      <MDiv variants={itemVariants}>
        <GlassCard 
          ...
          animate={false}
          className="p-4 border border-slate-200 dark:border-white/10 flex items-center justify-between group cursor-pointer hover:border-slate-300 dark:hover:border-white/20 transition-all bg-white dark:bg-black/40"
        >
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-neon-red to-rose-600 p-0.5">
          ...
          <div className="flex items-center gap-1 text-slate-400 dark:text-white/40 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">
            <span className="text-[10px] font-bold uppercase tracking-widest">Edit information</span>
      ...
      <MDiv variants={itemVariants}>
        <GlassCard animate={false} className="p-4 border border-slate-200 dark:border-white/10 flex items-center justify-between bg-white dark:bg-black/40">
          <div className="flex items-center gap-4">
      ...
      <MDiv variants={itemVariants} className="grid grid-cols-2 md:grid-cols-4 gap-px bg-slate-200 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden">
        {/* Position Value */}
        <div className="bg-white dark:bg-dark-950 p-6 flex flex-col items-center text-center space-y-3">
        ...
        {/* My Profit */}
        <div className="bg-white dark:bg-dark-950 p-6 flex flex-col items-center text-center space-y-3 border-l border-slate-100 dark:border-white/5">
          <div className="w-10 h-10 rounded-full bg-neon-red/10 flex items-center justify-center text-neon-red mb-1">
            <TrendingUp size={20} />
          </div>
          <div className="space-y-0.5">
      ...
      <MDiv variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <GlassCard 
          ...
          animate={false}
          className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-white/5 transition-colors cursor-pointer border-slate-200 dark:border-white/10 bg-white dark:bg-black/40"
        >
      ...
      <MDiv variants={itemVariants} className="space-y-2">
        {[
          ...
          <GlassCard key={idx} animate={false} className="p-4 border border-slate-200 dark:border-white/5 flex items-center gap-4 hover:border-slate-300 dark:hover:border-white/15 transition-all cursor-pointer bg-white dark:bg-black/40">
